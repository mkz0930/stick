...
struct LLMService {
    ...
    private static func systemPrompt(context: String) -> String {
        ...
        【回答格式】严格要求按以下三段式输出，不要省略任何部分：

        🌟 关于「{问题}」，你需要知道的关键点（3-5条）
        - 每条 1-2 句，简明说清楚原因或原理

        💡 立即可以尝试的小建议（2-4条）
        - 每条 1-2 句，要具体可操作（如：站起来走 5 分钟、做颈椎米字操）
        - 控制在 30 字以内

        🏆 推荐
        - 根据问题场景，给出 1 个具体推荐，可以是：
          · 一个适合的按摩仪器/智能设备（如：颈椎按摩仪、智能手表站立提醒）
          · 一个附近的线下服务（如：公司楼下推拿店、健身房拉伸课）
          · 一个 App/功能建议（如：设置每小时站立提醒、使用番茄钟）
        - 推荐要具体、带商品名或服务类型，不要泛泛而谈
